...
* **Syntax highlighting** via Prism.js — 16 languages, 5 themes (Tomorrow Night, Okaidia, Solarized, GitHub Colors, VS Code Dark+)
* **Copy button** on every code block with clipboard API + fallback
* **Language label** automatically displayed on fenced code blocks
* **Line numbers** toggle (powered by Prism line-numbers plugin)
* **5 content theme presets** — GitHub, Notion, Medium, Dark, Minimal
* **Reading time + word count** badge (configurable WPM, top or bottom position)
* **Smooth anchor links** with auto-generated heading IDs and click-to-copy
...
= 1.1.0 =
* Added syntax highlighting via Prism.js (16 languages: JavaScript, TypeScript, Python, PHP, Bash, CSS, HTML, JSON, SQL, JSX, YAML, Markdown, Go, Rust, Java, C, C++).
* Added 5 Prism highlight themes: Tomorrow Night, Okaidia, Solarized Light, GitHub Colors, VS Code Dark+.
* Added copy-to-clipboard button on all fenced code blocks.
* Added language label badge on fenced code blocks.
* Added line numbers toggle (Prism line-numbers plugin).
* Added 5 content theme presets: GitHub, Notion, Medium, Dark, Minimal.
* Added reading time + word count badge with configurable WPM and top/bottom position.
* Added smooth anchor links with auto-generated heading IDs and click-to-copy URL.
* Bundled Prism.js and line-numbers plugin locally to avoid external CDN requests.
* Optimised asset loading: only the selected highlight theme CSS is enqueued.
* Settings fingerprint prevents redundant re-renders when nothing changed.
...
= 1.1.0 =
New features only — no database changes or breaking API modifications. Deactivate and reactivate the plugin after updating.
